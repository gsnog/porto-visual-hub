...
const ContasReceber = () => {
  ...
  return (
    <div className="flex-1 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-6">Contas a Receber</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="bg-destructive border-destructive rounded-xl">
            <CardContent className="p-6">
              <div className="text-white">
                <h3 className="text-lg font-semibold mb-2">Total a Receber</h3>
        ...
        <div className="flex gap-4 mb-6">
          <Button className="rounded-full">Adicionar Conta</Button>
          <Button className="rounded-full" variant="outline">Relatório</Button>
        </div>

        {/* Filters */}
        <div className="flex gap-4 mb-6">
          <Input placeholder="Cliente" className="bg-[#efefef] !text-[#22265B] placeholder:!text-[#22265B] placeholder:opacity-100 h-10 px-3 w-64 rounded-full" />
          ...
          <Button className="h-10 px-3 w-64 rounded-full">Filtrar</Button>
        </div>
      </div>
          ...
          <Table className="table-custom">
            <TableHeader>
              <TableRow className="border-b bg-muted/50">
                ...
                <TableHead className="text-xs text-center">Encargos</TableHead>
                <TableHead className="text-xs text-center">Desconto</TableHead>
                <TableHead className="text-xs text-center">Valor Total Recebido</TableHead>
            ...
            <TableBody>
              {contas.map((conta, index) => (
                <TableRow key={index} className={index % 2 === 0 ? "bg-muted/20" : "bg-background"}>
                  ...
                  <TableCell className="text-center text-sm">{conta.cliente}</TableCell>
                  ...
                  <TableCell className="text-center text-sm">{conta.encargos}</TableCell>
                  <TableCell className="text-center text-sm">{conta.desconto}</TableCell>
                  <TableCell className="text-center text-sm">{conta.valorTotalRecebido}</TableCell>
                  <TableCell className="text-center text-sm">{conta.totalParcelas}</TableCell>
                  <TableCell className="text-center text-sm">{conta.parcelasPagas}</TableCell>
                  <TableCell className="text-center text-sm">{conta.proximoVencimento}</TableCell>
                  <TableCell className="text-center text-sm">{conta.status}</TableCell>
