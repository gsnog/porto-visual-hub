...
const ContaBancaria = () => {
  ...
  const [viewItem, setViewItem] = useState<typeof mockContas[0] | null>(null);
              ...
              {filtered.length === 0 ? (<TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Nenhuma conta encontrada.</TableCell></TableRow>) : (
                filtered.map((c) => (
                  <TableRow key={c.id} className="hover:bg-table-hover transition-colors">
                    ...
                    <TableCell className="text-center"><TableActions onView={() => setViewItem(c)} onEdit={() => toast({ title: "Editar", description: `Editando "${c.banco}"...` })} onDelete={() => setDeleteId(c.id)} /></TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
