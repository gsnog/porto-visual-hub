...
const Unidades = () => {
  ...
  const [editItem, setEditItem] = useState<typeof mockUnidades[0] | null>(null);
              ...
              {filtered.length === 0 ? (<TableRow><TableCell colSpan={2} className="text-center py-8 text-muted-foreground">Nenhuma unidade encontrada.</TableCell></TableRow>) : (
                filtered.map((u) => (
                  <TableRow key={u.id} className="hover:bg-table-hover transition-colors">
                    ...
                    <TableCell className="text-center"><TableActions onView={() => setViewItem(u)} onEdit={() => { setEditItem(u); setEditNome(u.nome); }} onDelete={() => setDeleteId(u.id)} /></TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
      ...
      <Dialog open={!!editItem} onOpenChange={() => setEditItem(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { if (editItem) { setItems(prev => prev.map(i => i.id === editItem.id ? { ...i, nome: editNome } : i)); setEditItem(null); toast({ title: "Salvo", description: "Unidade atualizada." }); } }}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
